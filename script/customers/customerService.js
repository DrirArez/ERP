(function(){
    ...
    function CustomerService($q){
        ...
        function updateCustomer(customer) {
            var deferred = $q.defer();
            var query = "UPDATE customers SET name = ?, email = ?, adress = ?, city = ?, phone = ?, birthDate = ?  WHERE customer_id =  ?";
            connection.query(query,[customer.name,customer.email,customer.adress,customer.city,customer.phone,customer.birthDate, customer.customer_id], function (err,res) {
                if(err) deferred.reject(err);
                console.log(err);
